...
export function BulkPrintView({ invoices, facility }: BulkPrintViewProps) {
  ...
  return (
    <>
      ...
      {/* All Invoices - Each on its own page when printed */}
      <div className="space-y-8 print:space-y-0">
        {invoices.map((invoice, index) => (
          <div
            key={invoice.id}
            className="invoice-page bg-white rounded-lg shadow-sm border border-gray-200 print:shadow-none print:border-none print:rounded-none"
          >
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {invoice.monthly_rate > 0 && (
                      <tr>
                        <td className="px-4 py-3 text-gray-700">Monthly Care Services</td>
                        <td className="px-4 py-3 text-right text-gray-900 font-medium">
                          {formatCurrency(invoice.monthly_rate)}
                        </td>
                      </tr>
                    )}
                    {invoice.daily_rate && invoice.daily_rate_days && invoice.daily_rate_total && (
                      <tr>
                        <td className="px-4 py-3 text-gray-700">
                          Daily Care Services ({invoice.daily_rate_days} days @ {formatCurrency(invoice.daily_rate)}/day)
                        </td>
                        <td className="px-4 py-3 text-right text-gray-900 font-medium">
                          {formatCurrency(invoice.daily_rate_total)}
                        </td>
                      </tr>
                    )}
                    {invoice.line_1_desc && invoice.line_1_amount && (
                      <tr>
                        <td className="px-4 py-3 text-gray-700">{invoice.line_1_desc}</td>
                        <td className="px-4 py-3 text-right text-gray-900 font-medium">
                          {formatCurrency(invoice.line_1_amount)}
                        </td>
                      </tr>
                    )}
                    {invoice.line_2_desc && invoice.line_2_amount && (
                      <tr>
                        <td className="px-4 py-3 text-gray-700">{invoice.line_2_desc}</td>
                        <td className="px-4 py-3 text-right text-gray-900 font-medium">
                          {formatCurrency(invoice.line_2_amount)}
                        </td>
                      </tr>
                    )}
                    {invoice.line_3_desc && invoice.line_3_amount && (
                      <tr>
                        <td className="px-4 py-3 text-gray-700">{invoice.line_3_desc}</td>
                        <td className="px-4 py-3 text-right text-gray-900 font-medium">
                          {formatCurrency(invoice.line_3_amount)}
                        </td>
                      </tr>
                    )}
                  </tbody>
